...
def login_post(login: User, bg_task: BackgroundTasks):
    try:
        signup_json = jsonable_encoder(approved_users[login.id])
        bg_task.add_task(audit_log_transaction, touristId=str(login.id),
                         message="login")
...
def login_get(username: str, password: str, bg_task: BackgroundTasks):
