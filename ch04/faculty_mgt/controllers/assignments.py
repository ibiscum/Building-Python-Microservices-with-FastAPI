...
def create_workbin(stud_id: int, faculty_id: int):
    assignment_submission_service: AssignmentSubmissionService = \
        AssignmentSubmissionService()
    result, bin_id = assignment_submission_service.create_workbin(
        stud_id, faculty_id)
    if result is True:
        return JSONResponse(content={'message': 'workbin %s has been created \
                                     successfuly' % (bin_id)}, status_code=201)
    else:
        return JSONResponse(content={'message': 'submission problem \
