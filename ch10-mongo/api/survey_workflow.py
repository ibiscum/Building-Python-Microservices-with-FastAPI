...
async def chained_workflow(surveydata: SurveyDataResult):
    ...
    print(survey_dict)
    result = chain(
        compute_sum_results.s(survey_dict["results"]).set(queue="default"),
        compute_avg_results.s(len(survey_dict)).set(queue="default"),
        derive_percentile.s().set(queue="default"),
    ).apply_async()
...
async def grouped_workflow(surveydata: SurveyDataResult):
    ...
    result = group(
        [
            save_result_xlsx.s(survey_dict["results"]).set(queue="default"),
            save_result_csv.s(len(survey_dict)).set(queue="default"),
        ]
    ).apply_async()
    return {"message": result.get(timeout=10)}
...
async def process_surveys(surveys: List[SurveyDataResult]):
    ...
    result = group(
        [
            chain(
                compute_sum_results.s(survey["results"]).set(queue="default"),
