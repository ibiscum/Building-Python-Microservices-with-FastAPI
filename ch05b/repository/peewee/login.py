...
from models.data.peewee_models import (
    Login,
    Profile_Trainers,
    Gym_Class,
    Profile_Members,
...
class LoginRepository:
    def insert_login(
        self, id: int, user: str, passwd: str, approved: date, type: int
    ) -> bool:
        try:
            Login.create(
                id=id,
...
class MemberGymClassesRepository:
    def outer_join_member_gym(self):
        return list(
            Profile_Members.select(Profile_Members, Gym_Class).join(
                Gym_Class, join_type=JOIN.LEFT_OUTER
            )
        )
