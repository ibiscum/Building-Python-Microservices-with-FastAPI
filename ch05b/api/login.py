...
from repository.peewee.login import (
    LoginRepository,
    LoginTrainersRepository,
    MemberGymClassesRepository,
)
...
async def add_login(req: LoginReq):
    ...
    result = repo.insert_login(
        req.id, req.username, req.password, req.date_approved, req.user_type
    )
    if result == True:
        return req
    else:
