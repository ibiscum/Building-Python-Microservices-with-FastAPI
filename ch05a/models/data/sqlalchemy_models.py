...
class Login(Base):
    ...
    trainers = relationship("Profile_Trainers", back_populates="login",
                            uselist=False)
    members = relationship("Profile_Members", back_populates="login",
                           uselist=False)
...
class Profile_Members(Base):
    ...
    trainer_id = Column(
        Integer, ForeignKey("profile_trainers.id"), unique=False, index=False
    )
...
class Attendance_Member(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    member_id = Column(
        Integer, ForeignKey("profile_members.id"), unique=False, index=False
    )
