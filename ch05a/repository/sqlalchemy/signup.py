...
class SignupRepository:
    ...
    def get_all_signup_where(self, username: str):
        return (
            self.sess.query(Signup.username, Signup.password)
            .filter(Signup.username == username)
            .all()
        )
...
class LoginMemberRepository:
    ...
    def join_login_members(self):
        return (
            self.sess.query(Login, Profile_Members)
            .filter(Login.id == Profile_Members.id)
            .all()
        )
...
class MemberAttendanceRepository:
    ...
    def join_member_attendance(self):
        return (
            self.sess.query(Profile_Members, Attendance_Member)
            .join(Attendance_Member)
            .all()
        )
