...
def add_login_coroutine(req: LoginReq):
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "insert login profile problem encountered"},
            status_code=500,
        )
...
def update_login(id: int, req: LoginReq):
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "update login profile problem encountered"},
            status_code=500,
        )
...
def list_login_records():
    ...
    result = yield from asyncio.gather(
        count_login(login_data), build_user_list(login_data)
    )
