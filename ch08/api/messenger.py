...
async def send_messnger_details(req: MessengerReq):
    ...
    producer.send(
        "newstopic",
        bytes(str(json.dumps(messenger_dict, default=json_date_serializer)), "utf-8"),
    )
        ...
        while True:
            ...
            if not len(message.items()) == 0:
                for tp, records in message.items():
                    for rec in records:
                        messenger_dict = json.loads(
                            rec.value.decode("utf-8"),
                        ...
                        id = uuid4()
                        yield {
                            "event": "Added messenger status: {}, Received: {}".format(
                                result,
