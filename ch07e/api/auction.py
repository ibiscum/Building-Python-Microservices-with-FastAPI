...
def add_auction(
    req: AuctionsReq,
    current_user: Login = Security(get_current_valid_user, scopes=["auction_write"]),
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "create auction problem encountered"}, status_code=500
        )
...
def list_all_auction(
    current_user: Login = Security(get_current_valid_user, scopes=["auction_read"]),
    sess: Session = Depends(sess_db),
):
    repo: AuctionsRepository = AuctionsRepository(sess)
