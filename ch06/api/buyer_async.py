...
async def add_buyer(req: BuyerReq, db=Depends(create_db_collections)):
    ...
    if result is True:
        ...
        return JSONResponse(
            content={"message": "add buyer unsuccessful"}, status_code=500
        )
...
async def delete_purchase_history(
    id: int, hist_id: int, db=Depends(create_db_collections)
):
    repo: BuyerRepository = BuyerRepository(db["buyers"])
...
async def add_customer_status(
    id: int, req: PurchaseStatusReq, db=Depends(create_db_collections)
):
...
async def update_buyer(id: int, req: BuyerReq, db=Depends(create_db_collections)):
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "update buyer unsuccessful"}, status_code=500
        )
...
async def delete_buyer(id: int, db=Depends(create_db_collections)):
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "delete buyer unsuccessful"}, status_code=500
        )
