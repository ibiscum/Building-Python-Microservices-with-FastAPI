...
from db_config.odmantic_config import (
    create_db_engine,
...
async def update_purchase(id: int, req: PurchaseReq, engine=Depends(create_db_engine)):
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "update purchase unsuccessful"}, status_code=500
        )
...
async def delete_purchase(id: int, engine=Depends(create_db_engine)):
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "delete purchase unsuccessful"}, status_code=500
        )
