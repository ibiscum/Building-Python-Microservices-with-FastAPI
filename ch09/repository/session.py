...
class DbSessionRepository:
    ...
    async def insert_session(self, details: Dict[str, Any]) -> bool:
        try:
            exist_sess = await self.engine.find_one(
                DbSession, DbSession.session_name == details["session_name"]
            )
            if exist_sess == None:
    ...
    async def delete_session(self, name: str) -> bool:
        try:
            keyword = await self.engine.find_one(
                DbSession, DbSession.session_name == name
            )
