...
async def add_restaurant(
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "insert login unsuccessful"}, status_code=500
        )
...
async def add_feedback(
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "add feedback unsuccessful"}, status_code=500
        )


@router.post("/restaurant/ambiance/rate/add")
async def add_ambiance_rate(
    req: AmbianceRateReq,
    id: int,
    engine=Depends(create_db_engine),
    user: str = Depends(get_current_user),
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "add ambiance rating unsuccessful"}, status_code=500
        )


@router.post("/restaurant/food/rate/add")
async def add_food_rate(
    req: FoodRateReq,
    id: int,
    engine=Depends(create_db_engine),
    user: str = Depends(get_current_user),
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "add food rating unsuccessful"}, status_code=500
        )
...
async def list_restaurant_names(
...
@router.get(
    "/restaurant/upload/video",
    responses={
        200: {
            "content": {"video/mp4": {}},
            "description": "Return an MP4 encoded video.",
        },
        500: {"model": Error500Model, "description": "The item was not found"},
    },
)
...
async def send_enc_login(
    engine=Depends(create_db_engine), user: str = Depends(get_current_user)
):
    repo: RestaurantRepository = RestaurantRepository(engine)
    result = await repo.get_all_restaurant()
