...
async def authenticate(
    response: Response,
    username: str = Query(
        ..., description="The username of the credentials.", max_length=50
    ),
    password: str = Query(
        ..., description="The password of the of the credentials.", max_length=20
    ),
...
async def add_login_profile(
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "insert purchase unsuccessful"}, status_code=500
        )
...
async def get_login(
    id: int = Path(..., description="The user ID of the user."),
    engine=Depends(create_db_engine),
    user: str = Depends(get_current_user),
):
    repo: LoginRepository = LoginRepository(engine)
