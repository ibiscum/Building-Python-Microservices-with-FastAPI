...
def insert_recipe_keywords(rid: UUID, keywords: List[str],
    ...
    if keywords is not None:
        ...
        return JSONResponse(content={"message": "inserted recipe keywords"},
                            status_code=201)
    else:
...
@inject
def add_recipe_keyword(rid: UUID, keyword: str, keywordservice:
                       KeywordRepository =
                       Depends(Provide[Container.keywordservice])):
...
@inject
def get_recipe_keywords(rid: UUID, keywordservice: KeywordRepository =
                        Depends(Provide[Container.keywordservice])):
...
@inject
def get_all_keywords(keywordservice: KeywordRepository =
                     Depends(Provide[Container.keywordservice])):
