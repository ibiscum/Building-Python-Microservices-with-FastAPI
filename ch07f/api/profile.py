...
def add_profile(
    ...
    current_user: Login = Security(
        get_current_valid_user, scopes=["bidder_write", "buyer_write"]
    ),
    ...
    if result == True:
        ...
        return JSONResponse(
            content={"message": "create profile problem encountered"}, status_code=500
        )
...
def delete_profile(
    ...
    if result:
        return JSONResponse(
            content={"message": "signup updated successfully"}, status_code=201
        )
    else:
...
def list_all_profile(
    current_user: Login = Security(
        get_current_valid_user, scopes=["bidder_read", "buyer_read"]
    ),
    sess: Session = Depends(sess_db),
):
    repo: ProfileRepository = ProfileRepository(sess)
