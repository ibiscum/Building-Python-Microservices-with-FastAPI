...
from models.request.library import BookRequestReq, BookReturnReq
...
def list_all_books():
    with requests.Session() as sess:
        response = sess.get("http://localhost:8001/ch04/library/book/list")
...
def list_all_request():
    with requests.Session() as sess:
        response = sess.post(
            "http://localhost:8001/ch04/library/book/request/list",
        )
        return response.json()
...
def request_borrow_book(request: BookRequestReq):
    with requests.Session() as sess:
        response = sess.post(
            "http://localhost:8001/ch04/library/book/request",
...
def return_book(returning: BookReturnReq):
    with requests.Session() as sess:
        response = sess.post(
            "http://localhost:8001/ch04/library/book/issuance/return",
